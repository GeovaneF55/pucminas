...
class Lista {
	 ...
	 */
	public void ordenacaoShellsort() {
		int h = 1;
		do {
			h = (h * 3) + 1;
		} while (h < getQtde());
	...
	void insercaoPorCor(int cor, int h){
		for (int i = (h + cor); i < getQtde(); i+=h) {
			Pessoa tmp = array[i].clone();
			int j = i - h;
			while ((j >= 0) && array[j].getDataNascimento().compareTo(tmp.getDataNascimento())>0) {
				TP05Q06Shellsort.COMPARACOES+=1;
				array[j + h] = array[j].clone();
				TP05Q06Shellsort.MOVIMENTACOES+=1;
				j -= h;
			}
			array[j + h] = tmp.clone();
			TP05Q06Shellsort.MOVIMENTACOES+=1;
