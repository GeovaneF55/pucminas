/**
 * TP05Q05 - Ordenação por Inserção
...
class TP05Q05OrdenacaoPorInsercao{
	...
	public void ordenacaoInsercao(){
		for (int i = 1; i < getQtde(); i++) {
			...
			while ((j >= 0) && (array[j].getNome().compareTo(tmp.getNome()) > 0)){
				TP05Q05OrdenacaoPorInsercao.COMPARACOES+=1;
				array[j + 1] = array[j].clone();
				TP05Q05OrdenacaoPorInsercao.MOVIMENTACOES+=1;
				j--;
			}
			array[j + 1] = tmp.clone();
			TP05Q05OrdenacaoPorInsercao.MOVIMENTACOES+=1;
