...
class Lista {
	...
	/**
	 * Retorna o atributo ordenado
	 * @return ordenado Pessoa[]
	 */
	...
	public void ordenacaoCountingsort() {
		...
		for (int i = 0; i < getQtde(); i++){
			int index = getDoidao(i) + Math.abs(menor);
			count[index]++;
		}
		...
		for (int i = getQtde()-1; i >= 0; i--){
			int indexCount = getDoidao(i) + Math.abs(menor),
			    indexOrdenado = count[indexCount]-1;

			ordenado[indexOrdenado] = array[i].clone();
			TP05Q09Countingsort.MOVIMENTACOES+=1;
			count[indexCount]--;
		}
		//Coloca os elementos ordenados de volta no array
		for(int i = 0; i < getQtde(); i++){
			array[i] = ordenado[i].clone();
			TP05Q09Countingsort.MOVIMENTACOES+=1;
		}
	}
}
