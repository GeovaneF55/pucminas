...
class Lista {
	...
	public void ordenacaoQuicksort(){
		ordenacaoQuicksort(0, getQtde()-1);
	}
	
	public void ordenacaoQuicksort(int esq, int dir){
		int i = esq,
		    j = dir;
                Pessoa pivo = array[(dir+esq)/2].clone();
		while (i <= j) {
			while (i < getQtde() && pivo.ehMaisVelho(array[i])){ //array[i] < pivo
				i++;
				TP05Q08Quicksort.COMPARACOES+=1;
			}
			while (j >= 0 && array[j].ehMaisVelho(pivo)){ //array[j] > pivo
				j--;
				TP05Q08Quicksort.COMPARACOES+=1;
			}
			if (i <= j){
	...
	public void swap(int menor, int i) {
		...
		TP05Q08Quicksort.MOVIMENTACOES+=3;
	}
}
