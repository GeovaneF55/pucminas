...
class Pessoa{
	 ...
	 */
	public boolean morreuAntes(Pessoa pessoa){
		int dataP1 = dataPadrao(this.getDataMorte());
		int dataP2 = dataPadrao(pessoa.getDataMorte());
		boolean morreuAntes = (dataP1 > dataP2),
			idMaiorMesmaIdadeMorte = (dataP1 == dataP2 && this.getId() > pessoa.getId());

		return morreuAntes || idMaiorMesmaIdadeMorte;
	}
	 ...
	 * @return data String
	 */
	public int dataPadrao(String data){
		int dataPadrao;		
		String []dataArray = data.split(" de ");

		if(data.contains("ca.")){ dataArray = new String[3]; dataArray[2] = "-" + somenteNumeros(data); dataArray[1] = "00"; dataArray[0] = "00";}

		try{
			dataPadrao = Integer.parseInt(dataArray[2] + parseMes(dataArray[1]) + dataArray[0]);
		} catch(ArrayIndexOutOfBoundsException exception){
			dataPadrao = Integer.parseInt("0000" + "00" + "00");
		}
	...
	/**
	 * Retorna o mês numérico a partir do mês por extenso
	 * @param mesExtenso String
	 * @return mesNumerico String
	 */
	public String parseMes(String mesExtenso){
		String mesNumerico;
		switch(mesExtenso){
			case "janeiro":
				mesNumerico = "01";
...
class Lista {
	...
	private Pessoa[] arrayTemporario;
	 ...
	 */
	public Pessoa[] getArrayTemporario(){
		return this.arrayTemporario;
	}
	...
	public void ordenacaoMergesort(){
		arrayTemporario = new Pessoa[getQtde()];
		fazMergesort(0, getQtde()-1);
	}

	/**
	 * Faz Merge
	 * @param esquerda int Indice Inferior.
	 * @param direita int Indice Superior.
	 */
	public void fazMergesort(int esquerda, int direita) {
		if(esquerda < direita) {
			int meio = (esquerda + direita)/2;
			fazMergesort(esquerda, meio);
			fazMergesort(meio + 1, direita);
			mergePartes(esquerda, meio + 1, direita);
		}
	}
	 ...
	 * @param direita int Indice Superior.
	 */
	public void mergePartes(int esquerda, int direita, int direitaFim) {

		int esquerdaFim = direita - 1;
		int k = esquerda;
		int num = direitaFim - esquerda + 1;

		while(esquerda <= esquerdaFim && direita <= direitaFim){
			if(array[direita].getDataMorte().compareTo(array[esquerda].getDataMorte())>0){
				arrayTemporario[k++] = array[esquerda++].clone();
			} else if(array[direita].getDataMorte().compareTo(array[esquerda].getDataMorte())==0 && array[direita].getId() > array[esquerda].getId()){
				arrayTemporario[k++] = array[esquerda++].clone();
			} else{
				arrayTemporario[k++] = array[direita++].clone();
			}
			TP05Q11Mergesort.MOVIMENTACOES+=1;
			TP05Q11Mergesort.COMPARACOES+=1;
		}
		
		while(esquerda <= esquerdaFim){
			arrayTemporario[k++] = array[esquerda++].clone();
			TP05Q11Mergesort.MOVIMENTACOES+=1;
		}

		while(direita <= direitaFim){
			arrayTemporario[k++] = array[direita++].clone();
			TP05Q11Mergesort.MOVIMENTACOES+=1;
		}

		for(int i = 0; i < num; i++, direitaFim--){
			array[direitaFim] = arrayTemporario[direitaFim].clone();
			TP05Q11Mergesort.MOVIMENTACOES+=1;
		}
