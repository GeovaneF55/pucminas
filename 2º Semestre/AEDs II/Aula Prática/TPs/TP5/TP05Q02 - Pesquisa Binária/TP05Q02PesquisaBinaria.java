...
class TP05Q02PesquisaBinaria{
	...
	public static void main(String[] args) throws Exception{
		...
		Pessoa pessoa;
		for(String nomeArquivo = MyIO.readLine(); nomeArquivo.equals("####") == false; nomeArquivo = MyIO.readLine()){
			pessoa = new Pessoa();
		...
		COMECO = now();
		for(String id = MyIO.readLine(); id.equals("FIM") == false; id = MyIO.readLine()){
                        if(lista.pesquisaBinaria(Integer.parseInt(id))){
				MyIO.println("SIM");
			} else{
				MyIO.println("NAO");
			}
...
class Lista {
	...
	public boolean pesquisaBinaria(int id) {
		...
		while (esq <= dir) {
			...
			TP05Q02PesquisaBinaria.COMPARACOES+=1;
		}
		return retorno;
	}
