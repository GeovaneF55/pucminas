/**
 * TP06Q02 - Lista com Alocação Flexível
 * @author Geovane Fonseca de Sousa Santos
 * @version 1 10/2016
 */

class TP06Q01ListaComAlocacaoFlexivel
{
...
class Celula {
	public Pessoa pessoa; // Pessoa inserida na celula.
...
class Lista {
	...
	public Pessoa removerFim() throws Exception {
		...
		Pessoa resp = ultimo.pessoa.clonar();
		ultimo = i; 
		i = ultimo.prox = null;
	...
	public Pessoa remover(int pos) throws Exception {
		...
		if (primeiro == ultimo){
			...
			resp = removerInicio().clonar();
		} else if (pos == tamanho - 1){
			resp = removerFim().clonar();
		} else {
			// Caminhar ate a posicao anterior a insercao
			Celula i = primeiro;
			for(int j = 0; j < pos; j++, i = i.prox);
	...
	public boolean pesquisar(Pessoa pessoa) {
		boolean resp = false;
		for (Celula i = primeiro.prox; i != null; i = i.prox) {
			if(i.pessoa.getNome().equals(pessoa.getNome())){
				resp = true;
				i = ultimo;
			}
		}
