class Principal {
    public static void main(String[] args) throws Exception {
        ...
        a2.inserir("palavra");
        Arvore concatenada = Arvore.insereIguais(arvore, a2);
    
        MyIO.println("\n ===== INSERINDO IGUAIS =====");
        if(concatenada.pesquisar("palavra") == true) {
            MyIO.println("Elemento palavra encontrado.");
        } else {
            MyIO.println("Elemento palavra nao cadastrado.");
        }

        if(concatenada.pesquisar("word") == true) {
            MyIO.println("Elemento word encontrado.");
        } else {
            MyIO.println("Elemento word nao cadastrado.");
        }
    }
} 
