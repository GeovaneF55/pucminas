class Arvore {
     ...
     */
    public No buscarNo(String palavra) throws Exception {
    ...
    public static Arvore insereIguais(Arvore a1, Arvore a2) throws Exception {
        Arvore resp = new Arvore();
        for(int i = (int) 'A'; i <= (int) 'Z'; i++) {
            No xr = resp.buscarNo(Character.toString((char) i));
            No x1 = a1.buscarNo(Character.toString((char) i));
            No x2 = a2.buscarNo(Character.toString((char) i));
    
            pesquisarIgual(x1.getRaiz(), a2, resp);
             
        }
        return resp;
    }   

    private static void pesquisarIgual(No2 n1, Arvore a2, Arvore resp) throws Exception {
        if(n1 != null) {
            pesquisarIgual(n1.getEsq(), a2, resp);
            if(a2.pesquisar(n1.getElemento()) == true) {
                resp.inserir(n1.getElemento());
            }
            pesquisarIgual(n1.getDir(), a2, resp);
        }
    }
}
