...
void mergesortParalelo(int arr[], int size){
	int mid;
	if(size > 1) {
		mid = size / 2;
		#pragma omp parallel sections
		{
			#pragma omp section
			{
				mergesortParalelo(arr, mid);
			}
			#pragma omp section
			{
				mergesortParalelo(arr + mid, size - mid);
			}
		}
		Merge(arr, size);
	}
