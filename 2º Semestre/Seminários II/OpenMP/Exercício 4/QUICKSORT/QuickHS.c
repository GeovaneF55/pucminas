...
void quickParalelo(int *a, int left, int right) {
	if (left < right)
	{
		int pivot = (left + right) / 2;
		int pos = partition(a,left,right,pivot);
		#pragma omp parallel sections
		{
			#pragma omp section
			{
				quickParalelo(a, left, pos - 1);
			}
			#pragma omp section
			{
				quickParalelo(a, pos + 1, right);
			}
		}
	}
}
