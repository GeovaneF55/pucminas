...
// Code for Begin, Abort, and Commit opcodes follows
...
OP(tmBegin_op_0){
  ...
  new transactionContext(pthread,picode);
...
PFPI tmBegin_op[] = { tmBegin_op_0, 0};
...
OP(tmAbort_op_0){
...

  {
    ...
    case 0: printf("CodeFlag: %d -> %d\n",pthread->getPid(),picode->immed);break;

    case 1: printf("CodeFlag: %d -> %d\n",pthread->getPid(),picode->immed);break;

    case 2: printf("CodeFlag: %d -> %d\n",pthread->getPid(),picode->immed);break;

    case 3: printf("CodeFlag: %d -> %d\n",pthread->getPid(),picode->immed);break;

    case 4: printf("CodeFlag: %d -> %d\n",pthread->getPid(),picode->immed);break;

    case 5: printf("CodeFlag: %d -> %d\n",pthread->getPid(),picode->immed);break;

    default: printf("CodeFlag: %d -> %d\n",pthread->getPid(),picode->immed);break;
...
PFPI tmAbort_op[] = { tmAbort_op_0, 0};
...
OP(tmCommit_op_0){
  ...
  pthread->transContext->commitTransaction(pthread, picode);
...
PFPI tmCommit_op[] = { tmCommit_op_0, 0};
...

{
  ...
  sysCall->exec(pthread,picode);
    ...
    // We must intercept the normal mint_read instruction here and pass it to

    // the transactional context since it directly accesses memory
...

    {
      ...
      RAddr buffBegin = pthread->virt2real(buf);

      if(err > 0)

        pthread->transContext->cacheReadBuffer(pthread, picode, tempBuff,buffBegin,r6);
...
OP(mint_write){
...

  {
    ...
    // We must intercept the normal mint_write instruction here and pass it to
  ...
  if(pthread->tmDepth > 0){
    ...
    char buffPoint[count];

    RAddr buffBegin = pthread->virt2real(buf);

    pthread->transContext->cacheWriteBuffer(pthread, picode, buffPoint,buffBegin,count);

    err=write(fd,buffPoint,count);

  }

  else{

    const void * buffPoint = (const void *)pthread->virt2real(buf);
